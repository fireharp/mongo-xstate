...
export const userStateMachine = setup({
  guards: {
    continueTreatment: ({ context }) => {
      // Add your logic here to determine if treatment should continue
      return true;
    },
    shouldFinishProgram: ({ context }) => {
      // Add your logic here to determine if program should finish
      return true;
    }
  }
}).createMachine({
    ...
    Refill: {
      on: {
        next: [
          {
            target: "Appointment",
            guard: "continueTreatment"
          },
          {
            target: "Finish Program",
            guard: "shouldFinishProgram"
          },
        ],
      },
    },
